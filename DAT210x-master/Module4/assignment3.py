...
#
# .. your code here ..
df_selection = df.drop('classification',axis=1)
...
#
# .. your code here ..
df_selection = df_selection.apply(pd.to_numeric, errors='ignore')

# Convert categorical features into booleans
df_selection = pd.get_dummies(df_selection)
...
if scaleFeatures: df_selection = helper.scaleFeatures(df_selection)
...
pca.fit(df_selection)
  
# transform to 2D
T = pca.transform(df_selection)
...
ax = helper.drawVectors(T, pca.components_, df_selection.columns.values, plt, scaleFeatures)
T = pd.DataFrame(T)
